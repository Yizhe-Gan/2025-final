...
def post_student(session, url, row, timeout=10):
    # Extract fields in a forgiving way like upload_students.py would
    sid = row.get('student_id') or row.get('id') or ''
    name = row.get('name') or ''
    email = row.get('email') or ''
    credits = row.get('credits') or row.get('credit') or '0'
...
def main():
    ...
    parser.add_argument('--csv', default='example_students.csv')
    ...
    url = args.api.rstrip('/') + '/student/add'
    ...
    def worker(row):
        sid = row.get('student_id') or row.get('id') or ''
        status, resp = post_student(session, url, row, timeout=args.timeout)
        nonlocal done
        with done_lock:
            done += 1
            current = done
        return (sid, status, resp, current)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futures):
            sid, status, resp, current = fut.result()
            if status is None:
                failures += 1
                print(f'[{current}/{total}] {sid} FAILED: {resp}')
            elif 200 <= status < 300:
                successes += 1
                if current % 100 == 0 or current <= 10:
                    print(f'[{current}/{total}] {sid} OK ({status})')
            else:
                failures += 1
                print(f'[{current}/{total}] {sid} ERROR ({status}): {resp}')
